(() => {
  ...
  const state = {
    ...
    ball: { x: ARENA.width / 2, y: ARENA.height / 2, vx: 0, vy: 0, speed: SETTINGS.ball.speed, inPlay: false },
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (e.key === '1') { state.cpuRight = !state.cpuRight; }
    if (e.code === 'Space') {
  ...
  function update(dt) {
    ...
    // Player controls
    if (state.cpuRight) {
      // CPU controls left paddle with difficulty-based AI
    ...
    // Right paddle (default player) controlled by arrows
    ...
    if (state.ball.vx < 0 && state.ball.x - SETTINGS.ball.radius <= state.left.x + SETTINGS.paddle.width) {
      if (paddleBallIntersect(state.left.x, state.left.y)) {
        ...
        const speed = Math.min(state.ball.speed + SETTINGS.ball.speedInc, SETTINGS.ball.speedMax);
